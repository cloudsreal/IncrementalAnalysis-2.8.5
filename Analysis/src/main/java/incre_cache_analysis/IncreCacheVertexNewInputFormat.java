...
public class IncreCacheVertexNewInputFormat extends TextVertexInputFormat<IntWritable, CacheVertexValue, NullWritable> {
    ...
    protected CacheVertexValue getValue(String[] tokens) {
      ...
      if (nFlag || add_old_flag) { // m1, UA
        ...
        if (fact_str == null || fact_str.isEmpty() || fact_str.charAt(3) == '2') { // case : 1) new added node,
          ...
          cacheVertexValue = new CacheVertexValue(eFlag);
        } else if (eFlag && fact_str.charAt(3) == '0') { // eflag = true && in is null
          eFlag = false;
          cacheVertexValue = new CacheVertexValue(eFlag);
